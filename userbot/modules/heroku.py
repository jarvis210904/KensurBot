    ...
    if exe == "get":
        ...
        if variable != '':
            if variable in heroku_var:
                ...
                await var.edit("**That var doesn't exist.**")
                return True
        else:
        ...
        variable = var.pattern_match.group(2)
        if variable == '':
            await var.edit("**Specify the config var you want to delete.**")
            return False
        if variable in heroku_var:
            if BOTLOG:
                await var.client.send_message(
                    ...
                    "**Delete ConfigVar**:\n"
                    f"`{variable}`")
            await var.edit("**Deleted the specified config var.**")
            del heroku_var[variable]
        else:
            await var.edit("**That config var doesn't exist.**")
            return True
    ...
    if variable in heroku_var:
        if BOTLOG:
            await var.client.send_message(
                ...
                "**Change ConfigVar**:\n"
                f"`{variable}` = `{value}`")
        await var.edit("**Config var has been set successfully.**")
    else:
        if BOTLOG:
            await var.client.send_message(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(heroku_api + path, headers=headers) as r:
            ...
            await dyno.edit(
                ...
                f" • {AppHours} hour(s) {AppMinutes} minute(s) - {AppPercentage}%\n\n"
                "Dyno hours quota remaining this month:\n"
                f" • {hours} hour(s), {minutes} minute(s) - {percentage}%**")
            return True
